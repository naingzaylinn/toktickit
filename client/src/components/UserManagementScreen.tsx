...
export default function UserManagementScreen({ currentUser }: { currentUser: AuthUser }) {
  ...
  return <section>
    ...
    {loading ? <p role="status">Loading users...</p> : listFailed ? null : users.length === 0 ? <p>{search || role ? "No users match your search or filter." : "No users are available."}</p> : <div className="row g-3">{users.map(user => <div className="col-12 col-lg-6" key={user.id}><article className="card h-100 shadow-sm"><div className="card-body"><h2 className="h5 text-break">{user.name}</h2><p className="text-break mb-2">{user.email}</p><p className="mb-2">{label(user.role)} · {user.isActive ? "Active" : "Inactive"}{user.mustChangePassword ? " · Password change required" : ""}</p><div className="d-flex flex-wrap gap-2"><button className="btn btn-sm btn-outline-primary-green" disabled={busy} onClick={() => openEdit(user)}>Edit</button><button className="btn btn-sm btn-outline-primary-green" disabled={busy || user.id === currentUser.id && user.isActive} onClick={() => toggle(user)}>{user.isActive ? "Deactivate" : "Activate"}</button><button className="btn btn-sm btn-outline-primary-green" disabled={busy || user.id === currentUser.id} onClick={() => { setEditing(null); setPasswordTarget(user); setPassword(""); setConfirm(""); setError(""); setMessage(""); }}>Set initial password</button></div></div></article></div>)}</div>}
